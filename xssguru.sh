...
printf "$bold$blue ** Developed by GuruRoyal <3 ** \n\n$end"
...
usage(){
    printf "xssguru Usage:\n\n"
    printf "./xssguru.sh -d <target.com>\n"
    printf "./xssguru.sh -d <target.com> -b <blindxss.xss.ht>\n"
    printf "./xssguru.sh -d <target.com> -o xss_results.txt \n"
    printf "./xssguru.sh -d <target.com> -b <blindxss.xss.ht> -o xss_results.txt\n\n"
    exit 1;
}
...
while [ -n "$1" ]; do
        ...
        -o| --output)
            ...
            ;;
                -v|--version)
                        echo "Version of xssguru: $version"
                        exit 0 ;;
                *)
...
if [ -z "$blind" ] ; then
    echo "XSS Automation Started using xssguru.."
    dalfox file "$domain"_xss.txt -o $out
else
    echo "XSS Automation Started using xssguru with your Blind Payload.."
